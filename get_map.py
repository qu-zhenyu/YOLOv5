...
if __name__ == "__main__":
    ...
    image_ids = open(os.path.join(VOCdevkit_path, "VOC2007/ImageSets/Main/test.txt"),
                     encoding='utf-8').read().strip().split()
    image_nums = len(image_ids)  # 测试集的图像数量
    ...
    # -------------------------------------------------------#
    confidence_setbyself = 0.18  # 改！get_map判空时的置信度阈值!
    # -------------------------------------------------------#
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        nonEmptyNumMap = 0  # 改！测试集检测后的非空图像数量!
        for image_id in tqdm(image_ids):
            image_path = os.path.join(VOCdevkit_path, "VOC2007/JPEGImages/" + image_id + ".jpg")
            image = Image.open(image_path)
            if map_vis:
                image.save(os.path.join(map_out_path, "images-optional/" + image_id + ".jpg"))
            flagNonEmptyNum = yolo.get_map_txt(image_id, image, class_names, map_out_path, confidence_setbyself)
            if flagNonEmptyNum:
                nonEmptyNumMap += 1
        print("Get predict result done.")
        print("测试集的图像总数量", image_nums)
        print("测试集检测后的非空图像总数量", nonEmptyNumMap)
    ...
    if map_mode == 0 or map_mode == 3:
        print("Get map.")
        get_map(confidence_setbyself, MINOVERLAP, True, path=map_out_path)
        print("Get map done.")

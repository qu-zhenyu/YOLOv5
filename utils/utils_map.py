...
def get_map(confidence_setbyself, MINOVERLAP, draw_plot, path = './map_out'):
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            for idx, detection in enumerate(dr_data):
                ...
                # if score[idx] > 0.5:
                #     score05_idx = idx
                if score[idx] > confidence_setbyself:
                    score05_idx = idx
            ...
            if len(prec)>0:
                ...
                if rec[score05_idx] != 0 or prec[score05_idx] != 0:
                    print(text + "\t||\tscore_threhold={0:.2f} : ".format(confidence_setbyself) + " Recall=" +
                          "{0:.2f}%".format(rec[score05_idx]*100) + " ; Precision=" +
                          "{0:.2f}%".format(prec[score05_idx]*100) + " ; F1=" +
                          "{0:.2f}".format(F1[score05_idx]))

                # 改！同时记录sum_right目前求和情况
                sum_right = sum_right + rec[score05_idx]*gt_counter_per_class[class_name]
